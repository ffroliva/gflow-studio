...
function App() {
  ...
  useEffect(() => {
    // Under Windows `%LOCALAPPDATA%` is typical.
    // We request the Rust backend for assets. If dbPath is empty, the Rust code resolves it.
    const defaultWinPath = `C:\\Users\\ffrol\\AppData\\Local\\gflow-cli\\gflow.db`;
    setDbPath(defaultWinPath);
    addLog(`Database path set to default: ${defaultWinPath}`);
  }, [setDbPath]);
      ...
      <header className="h-14 border-b border-white/5 bg-[#09090b]/80 backdrop-blur-md flex items-center justify-between px-6 shrink-0 z-30">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-zinc-900/50 border border-white/5 rounded-lg px-2.5 py-1.5 text-xs text-zinc-400 font-mono">
            <Settings className="h-3.5 w-3.5 text-zinc-500 animate-spin" style={{ animationDuration: "12s" }} />
            ...
            <div className="grid grid-cols-3 gap-1 bg-zinc-950 border border-white/5 rounded-lg p-0.5">
              <button
                ...
                className={`py-1 rounded text-[10px] font-bold text-center interactive cursor-pointer ${
                  filter === "all" ? "bg-zinc-900 text-white" : "text-zinc-500 hover:text-zinc-300"
                }`}
              >
                All
              </button>
              <button
                ...
                className={`py-1 rounded text-[10px] font-bold text-center interactive cursor-pointer ${
                  filter === "video" ? "bg-zinc-900 text-white" : "text-zinc-500 hover:text-zinc-300"
                }`}
              >
                Video
              </button>
              <button
                ...
                className={`py-1 rounded text-[10px] font-bold text-center interactive cursor-pointer ${
                  filter === "image" ? "bg-zinc-900 text-white" : "text-zinc-500 hover:text-zinc-300"
                }`}
              >
                Image
              </button>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {filteredAssets.length === 0 ? (
              <div className="h-32 flex flex-col items-center justify-center text-center p-4">
                <span className="text-xs text-zinc-600 font-medium">No records found</span>
                <span className="text-[10px] text-zinc-700 font-mono mt-1">Check gflow connection</span>
              </div>
            ) : (
                    ...
                    <div className="mt-2 border-t border-white/5 pt-2 flex justify-end gap-1.5">
                      <button
                        onClick={(e) => {
                          ...
                          addLog(`Added asset ${asset.flow_media_id.substring(0, 8)} to timeline.`);
                        }}
        ...
        <aside className="w-80 border-l border-white/5 bg-[#09090b]/40 backdrop-blur-md flex flex-col shrink-0 overflow-hidden z-25">
          ...
          <div className="h-[220px] p-4 border-b border-white/5 flex flex-col shrink-0">
            <div className="flex items-center gap-2 mb-2 text-zinc-400">
              ...
              <span className="font-bold text-[10px] tracking-wider">RESOLVE VIEWPORT</span>
            </div>
              ...
              {selectedAsset ? (
                <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
                  ...
                  <span className="text-[9px] font-mono text-zinc-600 mt-1 bg-zinc-900 border border-zinc-800 rounded px-1.5 py-0.5">
                    {selectedAsset.kind.toUpperCase()} | {selectedAsset.width || 1280}x{selectedAsset.height || 720}
                  </span>
                ...
                <div className="text-center p-4">
                  <span className="text-xs text-zinc-600 font-semibold">Select media preview</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-[#060608]/90">
            <div className="h-9 border-b border-white/5 flex items-center px-4 gap-2 text-zinc-400 shrink-0 bg-zinc-950/20">
              ...
              <span className="font-bold text-[10px] tracking-wider">DAEMON LOGS</span>
            </div>

            <div className="flex-1 overflow-y-auto p-4 font-mono text-[10px] text-zinc-400 space-y-2 select-text">
              {logs.map((log, index) => (
                <div key={index} className="leading-relaxed break-all border-l-2 border-[#d9ff00]/10 pl-2">
                  {log}
                </div>
              ))}
            </div>
