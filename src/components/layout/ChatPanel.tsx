...
const mockCharacters: CharacterRecord[] = [
  ...
  },
  {
    ...
    voice: "Charon",
    personality:
      "Wise, slow-speaking, rich vocabulary, referencing ancient stars.",
    created_at: new Date().toISOString(),
  },
];
...
export const ChatPanel: React.FC = () => {
  const [characters, setCharacters] =
    useState<CharacterRecord[]>(mockCharacters);
  const [selectedChar, setSelectedChar] = useState<CharacterRecord>(
    mockCharacters[0],
  );
  ...
  useEffect(() => {
    const fetchRealCharacters = async () => {
      try {
        const response = await mcpClientInstance.callTool(
        ...
        if (
          ...
          const mapped: CharacterRecord[] = response.characters.map(
            (c: any) => ({
        ...
        console.warn(
          "Failed to fetch characters via MCP daemon, using fallback templates:",
          err,
        );
      }
    };
  ...
  const [messages, setMessages] = useState<Record<string, Message[]>>({
    "char-1": [
      {
        sender: "character",
        text: "System diagnostics clear. Rangefinder operational. What coordinates are we scanning next?",
        timestamp: "12:04 PM",
      },
      {
        sender: "user",
        text: "Tell me about the sector we just entered.",
        timestamp: "12:05 PM",
      },
      {
        sender: "character",
        text: "High electromagnetic interference. Ruins of a pre-collapse server stack are located at heading 240. I advise caution.",
        timestamp: "12:05 PM",
        hasAudio: true,
      },
    ],
    "char-2": [
      {
        sender: "character",
        text: "Salutations, traveler. The scrolls are laid out, and the ink is fresh. Which era shall we discuss?",
        timestamp: "Yesterday",
      },
    ],
  });
  ...
  const handleSend = () => {
    ...
    setTimeout(() => {
      const replies: Record<string, string> = {
        ...
        "char-2":
          "Ah, that is a question of profound depths. Let us look to the stellar maps.",
      };
  ...
  const handleCreateCharacter = (e: React.FormEvent) => {
    ...
    setMessages((prev) => ({
      ...
      [newChar.id]: [
        {
          sender: "character",
          text: `Greetings! I am ${newCharName}. Ready to assist.`,
          timestamp: "Just now",
        },
      ],
    }));
        ...
        {isCreating ? (
          <div className="flex-1 overflow-y-auto p-8 flex items-center justify-center">
            <form
              ...
              className="w-full max-w-md glass-panel-elevated p-6 rounded-2xl shadow-premium relative"
            >
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label
                    htmlFor="newCharPersonality"
                    className="block text-[10px] text-zinc-500 mb-1"
                  >
                    PERSONALITY PROTOCOL
                  </label>
                  <textarea
                    id="newCharPersonality"
                    rows={3}
            ...
            <div className="h-12 border-b border-white/5 flex items-center justify-between px-4 bg-app-bg/85 z-10 shrink-0">
              ...
              <button
                onClick={() => {
                  ...
                  const remain = characters.filter(
